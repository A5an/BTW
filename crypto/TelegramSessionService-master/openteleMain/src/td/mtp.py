...
class MTP(BaseObject):  # nocov
    ...
    class Config(BaseObject):
        ...
        def endpoints(
            ...
            results: Dict[MTP.DcOptions.Address, Dict[MTP.DcOptions.Protocol, List[MTP.DcOptions.Endpoint]]] = {}
            results[MTP.DcOptions.Address.IPv4] = {MTP.DcOptions.Protocol.Tcp: [], MTP.DcOptions.Protocol.Http: []}  # type: ignore
            results[MTP.DcOptions.Address.IPv6] = {MTP.DcOptions.Protocol.Tcp: [], MTP.DcOptions.Protocol.Http: []}  # type: ignore
            ...
            for endpoint in endpoints:
                ...
                if dcId == 0 or endpoint.id == dcId:
                    ...
                    address = MTP.DcOptions.Address.IPv6 if (flags & MTP.DcOptions.Flag.f_ipv6) else MTP.DcOptions.Address.IPv4
                    results[address][MTP.DcOptions.Protocol.Tcp].append(endpoint)  # type: ignore

                    if not (flags & (MTP.DcOptions.Flag.f_tcpo_only | MTP.DcOptions.Flag.f_secret)):
                        results[address][MTP.DcOptions.Protocol.Http].append(endpoint)  # type: ignore
